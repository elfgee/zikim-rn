...
export const Tooltip = <T,>({
	...
	style,
	...props
}: TooltipProps<T> & StaticPressableProps) => {
	...
	useEffect(() => {
		if (target && node && visible) {
			let pageWidth = 0
			let handle: any = findNodeHandle(node)
			// web일 경우 node가 scrollable한 컴포넌트(ScrollView, SectionList 등)일 경우 scrollable한 요소의 자식요소를 target으로 처리함.
			// 툴팁을 직접 scrollable 컴포넌트 바깥에서 선언해서 사용할 경우에는 childNodes[0] 를 보지 않아도 되므로 분기처리함.
			if (Platform.OS === "web" && usingInnerScrolllingView && handle && (handle as any).scrollToEnd) {
				handle = (handle as any).childNodes[0]
			}

			if (handle === null) return
			if (handle.measure) {
				handle.measure((x: number, y: number, width: number, height: number) => {
					pageWidth = width
				})
			}
			UIManager.measureLayout(
				//TODO Argument of type 'string' is not assignable to parameter of type 'number'.
				target as any,
				handle,
